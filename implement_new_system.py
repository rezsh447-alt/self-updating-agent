...
config["MAX_USERS"] = 6
config["AUTO_BACKUP_INTERVAL"] = 10  # هر 10 پیام
...
class AdvancedSessionManager:
    ...
    def get_user_chats(self, user_id):
        ...
        for chat_file in os.listdir(f"{user_chats_dir}/full"):
            if chat_file.endswith(".json"):
                chat_id = chat_file[:-5]
                chats[chat_id] = self.load_chat_summary(user_id, chat_id)
        return chats
    ...
    def generate_summary(self, chat_data):
        ...
        return {
            ...
            "summary": f"چت با {len(messages)} پیام. موضوع: {chat_data['title']}",
            "last_activity": chat_data["updated_at"],
            "message_count": chat_data["message_count"],
            "recent_messages": [msg.get("content", "")[:100] for msg in last_messages]
        }
    ...
    def add_message(self, user_id, chat_id, role, content):
        ...
        # بررسی نیاز به پشتیبان‌گیری در گیت‌هاب
        if chat_data["message_count"] % 10 == 0:  # هر 10 پیام
...
with open("session_manager.py", "w", encoding="utf-8") as f:
    f.write(NEW_SESSION_MANAGER)

# 4. ایجاد users.json
print("👥 در حال ایجاد users.json...")
with open("users.json", "w", encoding="utf-8") as f:
    json.dump(config["ALLOWED_USERS"], f, ensure_ascii=False, indent=2)

# 5. ایجاد فایل اولیه برای مدیر
print("👑 در حال ایجاد چت اولیه برای مدیر...")
manager = AdvancedSessionManager()
manager.create_chat(config["ADMIN_ID"], "چت اصلی")
